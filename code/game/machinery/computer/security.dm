				...
				for(var/datum/data/crime/p in active2.fields[DATACORE_CITATIONS])
					if(p.dataId == text2num(href_list["cdataid"]))
						...
						if(!pay)
							...
							to_chat(usr, span_notice("You have paid \u00a5[pay] towards your fine."))
...
<th>Time Added</th>
			...
			if("Print Missing")
				if(!( printing ))
					...
					if(missing_name)
						var/default_description = "A poster declaring [missing_name] to be a missing individual, missed by Sector Control. Report any sightings to security immediately."
